...
public class BaseExplorerAgent extends AbstractDedaleAgent {
    ...
    public float getProp() {
        ...
        if (totalSpace != 0)
            return (float)(this.totalSpace - available) / this.totalSpace;
        return 0;
    }
    ...
    public void updateTreasure(String Pos, Observation obs, int quantity, long t) {
        for (int i = 0; i < this.treasures.size(); i++) {
            if (this.treasures.get(i).getPosition().compareTo(Pos) == 0) {
    ...
    public void mergeTreasures(List<Treasure> t) {
        for (int j = 0; j < t.size(); j++) {
            ...
            if (!in) {
                ...
                this.addTreasure(new Treasure(tr.getPosition(), tr.getType(), tr.getQuantity(), tr.getObsTime()));
            }
        }
    }
